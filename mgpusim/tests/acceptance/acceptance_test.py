...
def main():
    ...
    hotspot = Test('../../samples/hotspot',
                   'hotspot',
                   ['-rows=256', '-cols=256', '-iterations=10', '-pyramid-height=1'],
                   '../../benchmarks/rodinia/hotspot')
    backprop = Test('../../samples/backprop',
                    'backprop',
                    ['-input=1024', '-iterations=2'],
                    '../../benchmarks/rodinia/backprop')
    lud = Test('../../samples/lud',
               'lud',
               ['-dim=128'],
               '../../benchmarks/rodinia/lud')
    gaussian = Test('../../samples/gaussian',
                    'gaussian',
                    ['-dim=128'],
                    '../../benchmarks/rodinia/gaussian')
    srad = Test('../../samples/srad',
                'srad',
                ['-rows=128', '-cols=128', '-iterations=10', '-lambda=0.5'],
                '../../benchmarks/rodinia/srad')
    diffusion = Test('../../samples/diffusion',
                     'diffusion',
                     ['-nx=32', '-ny=32', '-nz=32', '-iterations=4', '-alpha=0.05'],
                     '../../benchmarks/tartan/diffusion')
    tartan_hit = Test('../../samples/hit',
                      'hit',
                      ['-nx=24', '-ny=24', '-nz=24', '-iterations=3',
                       '-dt=0.01', '-nu=0.1'],
                      '../../benchmarks/tartan/hit')
    tartan_eqwp = Test('../../samples/eqwp',
                       'eqwp',
                       ['-nx=20', '-ny=20', '-nz=20', '-iterations=2'],
                       '../../benchmarks/tartan/eqwp')
    tartan_lulesh = Test('../../samples/lulesh',
                         'lulesh',
                         ['-num-node=8192', '-iterations=3',
                          '-dt=1e-7', '-u-cut=1e-7'],
                         '../../benchmarks/tartan/lulesh')
    tartan_pagerank = Test('../../samples/tartan-pagerank',
                           'tartan-pagerank',
                           ['-node=2048', '-edges-per-node=8',
                            '-iterations=6', '-damping=0.85'],
                           '../../benchmarks/tartan/pagerank')
    tartan_pennant = Test('../../samples/pennant',
                          'pennant',
                          ['-zones=2048', '-points=4096', '-min-pts=3',
                           '-max-pts=6', '-iterations=4', '-dt=0.01', '-gamma=1.4'],
                          '../../benchmarks/tartan/pennant')
    tartan_als = Test('../../samples/als',
                      'als',
                      ['-users=512', '-items=512', '-rank=8',
                       '-nnz-per-user=8', '-iterations=2', '-lambda=0.05'],
                      '../../benchmarks/tartan/als')
    lonestar_mst = Test('../../samples/mst',
                        'mst',
                        ['-nodes=2048', '-extra-edges=8192', '-iterations=12'],
                        '../../benchmarks/lonestar/mst')
    coral_snap = Test('../../samples/snap',
                      'snap',
                      ['-nx=64', '-ny=32', '-nz=32', '-iterations=4'],
                      '../../benchmarks/coral/snap')
    coral_nekbone10 = Test('../../samples/nekbone10',
                           'nekbone10',
                           ['-elements=64', '-iterations=3', '-lambda=0.1'],
                           '../../benchmarks/coral/nekbone10')
    ...
    if args.unified_multi_gpu:
        ...
        err |= hotspot.test(test_multi_gpu=False)
        err |= backprop.test(test_multi_gpu=False)
        err |= lud.test(test_multi_gpu=False)
        err |= gaussian.test(test_multi_gpu=False)
        err |= srad.test(test_multi_gpu=False)
        err |= diffusion.test(test_multi_gpu=False)
        err |= tartan_hit.test(test_multi_gpu=False)
        err |= tartan_eqwp.test(test_multi_gpu=False)
        err |= tartan_lulesh.test(test_multi_gpu=False)
        err |= tartan_pagerank.test(test_multi_gpu=False)
        err |= tartan_pennant.test(test_multi_gpu=False)
        err |= tartan_als.test(test_multi_gpu=False)
        err |= lonestar_mst.test(test_multi_gpu=False)
        err |= coral_snap.test(test_multi_gpu=False)
        err |= coral_nekbone10.test(test_multi_gpu=False)
