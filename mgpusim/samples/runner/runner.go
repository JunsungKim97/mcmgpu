...
var platformType = flag.String("platform-type", "mcmgpu",
	"Name of the platform to build (ideal, mcmgpu, i2, mi300, distlb, etc)")
...
func (r *Runner) Init() *Runner {
	...
	// r.addTLBLatencyTracer() // junsung
	// r.addTLBConditionalTracer() // junsung
	// r.addTLBCoalesceTracer() // junsung
	// r.addL2TLBMSHRLenTracer() // junsung
	r.addRTUCoalesceTracer()
	r.addTLBHitRateTracer()
	r.addDRAMLatencyTracer()
	r.addDRAMTracer()
	r.addPageWalkLatencyTracer()
	r.addMMUConditionalTracer() // junsung
	r.addAddressTranslatorLatencyTracer()
	r.addCacheHitRateTracer()
	// r.addPWCHitRateTracer() // junsung
	r.addRDMAEngineTracer()
	r.addCDMAEngineTracer()
	// r.addRTUTracer() // junsung
	// r.addActiveWalkerTracer() // junsung
	// r.addL2TLBQueueingImbalanceTracer() // junsung
	// r.addL2TLBEntropyTracer() // junsung
	// r.addRemoteReferenceTracer() // junsung
	// r.addTLBSetMissTracer() // junsung
	// r.addTLBMSHRStallTracer() // junsung
	// r.addTLBReqStallTracer() // junsung
	// r.addPageWalkerImbalanceTracker() // junsung
	// r.addL2TLBSQLTracer() // junsung
...
func (r *Runner) buildTimingPlatform() {
	...
	case "i2":
		...
		//junsung "2026-04-27" MI300 memory locality grouping for XCD local/remote routing.
		b.WithMemGroupSize(*memGroupSize)
		r.Engine, r.GPUDriver = b.Build()
	case "mi300":
		b := platform.MakeMI300GPUPlatformBuilder()
		if r.Parallel {
			b.WithParallelEngine()
		}
...
func (r *Runner) addCacheLatencyTracer() {
	...
	for _, gpu := range r.GPUDriver.GPUs {
		...
		//junsung "2026-04-27" Collect latency metrics from MI300 XCD private caches.
		for _, cache := range gpu.XCDCaches {
			tracer := tracing.NewAverageTimeTracer(
				func(task tracing.Task) bool {
					return task.Kind == "req_in"
				})
		...
		//junsung "2026-04-27" Reuse pipeline latency reporting for XCDCache pipeline too.
		for _, cache := range gpu.XCDCaches {
			pipeline := cache.GetPipeline()
			tracer := tracing.NewAverageTimeTracer(
				func(task tracing.Task) bool {
...
func (r *Runner) addCacheHitRateTracer() {
	...
	for _, gpu := range r.GPUDriver.GPUs {
		...
		//junsung "2026-04-27" Collect hit/miss metrics for XCDCache like other writeback caches.
		for _, cache := range gpu.XCDCaches {
			tracer := tracing.NewStepCountTracer(
				func(task tracing.Task) bool {
					return task.Kind == "req_in"
...
func (r *Runner) reportMemoryFootprint() {
	r.metricsCollector.Collect(
		r.GPUDriver.Name(),
		"memory_footprint_bytes",
		float64(r.GPUDriver.TotalMemoryFootprint()),
	)
}
