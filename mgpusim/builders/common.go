...
import (
	...
	"gitlab.com/akita/mem/cache/xcdcache"
...
func (b *CommonBuilder) WithMemGroupSize(n int) {
	b.memGroupSize = n
}
...
func (b *CommonBuilder) buildMemBanks(chiplet *Chiplet) {
	...
	// fmt.Printf("b.log2CacheLineSize: %d\n", b.log2CacheLineSize)
...
func (b *CommonBuilder) buildXCDCache(chiplet *Chiplet) {
	xcdBuilder := xcdcache.MakeBuilder().
		WithEngine(b.engine).
		WithFreq(b.freq).
		WithLog2BlockSize(b.log2CacheLineSize).
		WithWayAssociativity(16).
		...
		// WithNumMSHREntry(64).
		// WithNumReqPerCycle(2)
		WithNumReqPerCycle(32) // 32*1GHz*64B=2TB/s

	// fmt.Printf("b.log2CacheLineSize: %d\n", b.log2CacheLineSize)

	xcdName := fmt.Sprintf("%s.XCDCache", chiplet.name)
	xcd := xcdBuilder.Build(xcdName)
	b.xcdCaches = append(b.xcdCaches, xcd)
	b.gpu.XCDCaches = append(b.gpu.XCDCaches, xcd)
	chiplet.XCDCaches = append(chiplet.XCDCaches, xcd)

	if b.enableVisTracing {
		tracing.CollectTrace(xcd, b.visTracer)
	}
}
...
func (b *CommonBuilder) connectL1ToXCDCache(chiplet *Chiplet) {
	//junsung "2026-04-27" L1 계층의 모든 트래픽을 XCD private cache로 먼저 모은다.
	l1ToXCDConn := akita.NewDirectConnection(chiplet.name+".L1-XCD",
		b.engine, b.freq)

	for _, xcd := range chiplet.XCDCaches {
		l1ToXCDConn.PlugIn(xcd.TopPort, 64)
	}

	for _, l1v := range chiplet.L1VCaches {
		l1v.SetLowModuleFinder(&cache.SingleLowModuleFinder{
			LowModule: chiplet.XCDCaches[0].TopPort,
		})
		l1ToXCDConn.PlugIn(l1v.BottomPort, 16)
	}

	for _, l1s := range chiplet.L1SCaches {
		l1s.SetLowModuleFinder(&cache.SingleLowModuleFinder{
			LowModule: chiplet.XCDCaches[0].TopPort,
		})
		l1ToXCDConn.PlugIn(l1s.BottomPort, 16)
	}

	for _, l1iAT := range chiplet.L1IAddrTranslator {
		l1iAT.SetLowModuleFinder(&cache.SingleLowModuleFinder{
			LowModule: chiplet.XCDCaches[0].TopPort,
		})
		l1ToXCDConn.PlugIn(l1iAT.GetBottomPort(), 16)
	}

	chiplet.L1ToL2Connection = l1ToXCDConn
}

func (b *CommonBuilder) connectXCDCacheToL2(chiplet *Chiplet) {
	//junsung "2026-04-27" XCDCache 하단에서 mem-group-size 기준 local L2 / remote RDMA로 분기한다.
	if b.memGroupSize <= 0 {
		panic("mem-group-size must be >= 1")
	}
	groupID := int(chiplet.ChipletID) / b.memGroupSize
	localStartDie := groupID * b.memGroupSize
	localEndDie := localStartDie + b.memGroupSize - 1
	localStartBank := uint64(localStartDie * b.numMemoryBankPerChiplet)
	localEndBank := uint64((localEndDie+1)*b.numMemoryBankPerChiplet - 1)
	...
	xcdToL2Conn := akita.NewDirectConnection(chiplet.name+".XCD-L2",
		b.engine, b.freq)

	chiplet.chipRdmaEngine.SetLocalModuleFinder(lowModuleFinder)
	xcdToL2Conn.PlugIn(chiplet.chipRdmaEngine.ToL1, 64)
	xcdToL2Conn.PlugIn(chiplet.chipRdmaEngine.ToL2, 64)

	for _, l2 := range chiplet.L2Caches {
		lowModuleFinder.LowModules = append(lowModuleFinder.LowModules, l2.TopPort)
		xcdToL2Conn.PlugIn(l2.TopPort, 64)
	}

	for _, xcd := range chiplet.XCDCaches {
		xcd.SetLowModuleFinder(lowModuleFinder)
		xcdToL2Conn.PlugIn(xcd.BottomPort, 64)
	}
