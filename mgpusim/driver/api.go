...
func (d *Driver) AllocateMemory(
	...
	d.recordAllocation(ctx, byteSize)
	return GPUPtr(ptr)
}
...
func (d *Driver) AllocateUnifiedMemory(
	...
	ptr := d.memAllocator.AllocateUnified(ctx.pid, byteSize)
	d.recordAllocation(ctx, byteSize)
	return GPUPtr(ptr)
}
...
func (d *Driver) recordAllocation(ctx *Context, byteSize uint64) {
	allocated := d.pageAlignedSize(byteSize)
	total := atomic.AddUint64(&ctx.allocatedBytes, allocated)

	for {
		peak := atomic.LoadUint64(&ctx.peakAllocatedBytes)
		if total <= peak {
			return
		}

		if atomic.CompareAndSwapUint64(&ctx.peakAllocatedBytes, peak, total) {
			return
		}
	}
}

func (d *Driver) pageAlignedSize(byteSize uint64) uint64 {
	pageSize := uint64(1 << d.Log2PageSize)
	if byteSize == 0 {
		return 0
	}
	return ((byteSize-1)/pageSize + 1) * pageSize
}

// TotalMemoryFootprint returns the aggregated peak footprint of all contexts.
func (d *Driver) TotalMemoryFootprint() uint64 {
	var total uint64
	...
	for _, ctx := range d.contexts {
		total += atomic.LoadUint64(&ctx.peakAllocatedBytes)
	}
