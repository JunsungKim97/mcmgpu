...
// If numChiplets is not provided, 4 is used by default.
func (c *InterChipletConnector) CreateNetwork(numChiplets ...int) {
	n := 4
	if len(numChiplets) > 0 {
		n = numChiplets[0]
	}
	if n <= 0 {
		panic("numChiplets must be positive")
	}

	c.chipSwitch = switching.MultiFlitChipletSwitchBuilder{}.
		WithEngine(c.engine).
		WithFreq(c.freq).
		WithArbiter(arbitration.NewFlitAwareXBarArbiter(n)).
