...
def main():
    ...
    hotspot = Test(benchmarks_path + 'hotspot')
    ...
    backprop = Test(benchmarks_path + 'backprop')
    lud = Test(benchmarks_path + 'lud')
    gaussian = Test(benchmarks_path + 'gaussian')
    srad = Test(benchmarks_path + 'srad')
    diffusion = Test(benchmarks_path + 'diffusion')
    tartan_hit = Test(benchmarks_path + 'hit')
    tartan_eqwp = Test(benchmarks_path + 'eqwp')
    tartan_lulesh = Test(benchmarks_path + 'lulesh')
    tartan_pagerank = Test(benchmarks_path + 'tartan-pagerank')
    tartan_pennant = Test(benchmarks_path + 'pennant')
    tartan_als = Test(benchmarks_path + 'als')
    lonestar_mst = Test(benchmarks_path + 'mst')
    coral_snap = Test(benchmarks_path + 'snap')
    coral_nekbone10 = Test(benchmarks_path + 'nekbone10')
    ...
    err |= backprop.compile()
    err |= lud.compile()
    err |= gaussian.compile()
    err |= srad.compile()
    err |= diffusion.compile()
    err |= tartan_hit.compile()
    err |= tartan_eqwp.compile()
    err |= tartan_lulesh.compile()
    err |= tartan_pagerank.compile()
    err |= tartan_pennant.compile()
    err |= tartan_als.compile()
    err |= lonestar_mst.compile()
    err |= coral_snap.compile()
    err |= coral_nekbone10.compile()
