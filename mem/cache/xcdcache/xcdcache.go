...
// An XCDCache in xcdcache package  is a cache that performs the write-back policy
type XCDCache struct {
...
func (c *XCDCache) GetPipeline() pipelining.Pipeline {
	return c.pipeline
}

func (c *XCDCache) SetLowModuleFinder(lmf cache.LowModuleFinder) {
	c.lowModuleFinder = lmf
}

func (c *XCDCache) Tick(now akita.VTimeInSec) bool {
	madeProgress := false
...
func (c *XCDCache) runPipeline(now akita.VTimeInSec) bool {
	madeProgress := false
...
func (c *XCDCache) runStage(now akita.VTimeInSec, stage akita.Ticker) bool {
	madeProgress := false
...
func (c *XCDCache) discardInflightTransactions(now akita.VTimeInSec) {
